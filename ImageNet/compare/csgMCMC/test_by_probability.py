...
parser.add_argument('--network', type=str, choices=['resnet','resnet_bayesian', 'sdenet','mc_dropout','sdenet_multi','sdenet_bayesian'], default='resnet')
    ...
    with torch.no_grad():
        for data, targets in test_loader:
            ...
            for j in range(args.eva_iter):
                model.load_state_dict(torch.load('./save_csgmcmc_imagenet/imagenet_model_%i.pt'%(j))).to(device)
                model.eval()
