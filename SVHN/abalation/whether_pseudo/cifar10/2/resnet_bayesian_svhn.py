...
optimizer1 = optim.SGD([{'params':[ param for name, param in net.named_parameters() if 'bayesian' not in name]}], lr=0.001, momentum=0.9, weight_decay=5e-4)
...
def train(epoch):
    ...
    for (batch_idx, (inputs, targets)),(nt_batch_idx, (nt_inputs, nt_targets)) in zip(enumerate(train_loader),enumerate(nt_train_loader)):
    ...
    ##training with out-of-domain data
        ...
        loss_out = net.sample_elbo(inputs=nt_inputs,
                                       labels=label,
...
if not os.path.isdir('./save_resnet_bayesian_mnist'):
    os.makedirs('./save_resnet_bayesian_mnist')
torch.save(net.state_dict(),'./save_resnet_bayesian_mnist/final_model')
